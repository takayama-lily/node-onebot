...
function dipatch(event) {
    ...
    if (config.secret) {
        options.headers["X-Signature"] = crypto.createHmac("sha1", secret.toString()).update(json).digest("hex");
    }
...
async function onWSMessage(ws, data) {
    if (!online) {
        return ws.send(JSON.stringify({
            retcode: 103, status: "failed"
        }))
    }
    try {
        data = JSON.parse(data);
        if (data.action === ".handle_quick_operation") {
            var ret = await api.handleQuickOperation(data)
        } else {
            var ret = await api.apply(data);
        }
        ws.send(ret);
