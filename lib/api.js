...
async function quickOperate(event, res) {
    if (event.post_type === "message" && res.reply) {
        ...
        return await bot[action](id, res.reply, res.auto_escape);
    }
    if (event.post_type === "request" && res.hasOwnProperty("approve")) {
        ...
        return await bot[action](event.flag, res.approve, res.reason?res.reason:"", res.block?true:false);
    }
}

async function handleQuickOperation(data) {
    const event = data.params.context, res = data.params.operation;
    const ret = await quickOperate(event, res);
